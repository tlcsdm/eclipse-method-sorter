...
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.astextractor.AccessLevelComparatorExtractor;
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.astextractor.ConstructorComparatorExtractor;
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.astextractor.InitializerInvocationComparatorExtractor;
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.astextractor.SourcePositionComparatorExtractor;
import com.tlcsdm.eclipse.methodsorter.sorter.invocation.InvocationSorter;
...
public class ComparatorFactory {
	...
	public static Comparator<Signature> getInvocationComparator(NodeOrdering nodeOrdering,
		...
		final List<CallGraphNode> sortedNodes = traversalStrategyIsDepthFirst
				? invocationSorter.getOrderedListDepthFirst(callGraphNodes)
				: invocationSorter.getOrderedListBreadthFirst(callGraphNodes);
	 ...
	 * methods that are called from 5 different methods have the position 5. Note:
	 * Self-recursive methods are counted as being called by a method (check if this
	 * makes sense...).
	 *
	...
	/**
	 * Example: Method that calls some methods but is not called by itself has the
	 * position 0, is sorted first. A method that calls 5 methods but is called once
	 * itself, has the position 1/5. A method that is called some times but does not
	 * call anybody has the maximum position, is sorted as last method.
	 *
	 * @param callGraphNodes
	 * @return
	 */
	...
	/**
	 * Returns a signature comparator which prefers methods that call multiple other
	 * methods.
	 * <p>
	 * Example: A method that calls 5 other methods has the position -5, a method
	 * which calls 0 other methods has the position 0.
	 *
	 * @param callGraphNodes
	 * @return
	 */

...
public class NodeOrderingInterleaved implements NodeOrdering {
	...
	/**
	 * Inserts the given node v before a node w in the list only if w is reachable
	 * from v. If not, v is inserted at the end of the list.
	 *
	 * @param node
	 */
	@Override
	...
	public class ReachabilitiesWithoutBackEdges {
		 ...
		 * @return a set containing all nodes that are reachable from the given node
		 */
		private Set<CallGraphNode> getReachabilitySetWithoutBackEdges(CallGraphNode node,
				List<CallGraphNode> backNodes) {

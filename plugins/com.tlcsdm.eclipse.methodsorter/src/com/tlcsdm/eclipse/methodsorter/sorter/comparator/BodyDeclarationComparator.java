...
public class BodyDeclarationComparator implements Comparator<BodyDeclaration> {
	 ...
	 * @see CompilationUnitSorter#sort(int, org.eclipse.jdt.core.ICompilationUnit,
	 *      int[], java.util.Comparator, int,
	...
	private int category(BodyDeclaration bodyDeclaration) {
		...
		case ASTNode.METHOD_DECLARATION: {
			...
			if (method.isConstructor()) {
				...
				return getMemberCategory(1);
			} else {
				// MembersOrderPreferenceCache.METHOD_INDEX
				return getMemberCategory(2);
			}
		}
		case ASTNode.FIELD_DECLARATION: {
		...
		case ASTNode.INITIALIZER: {
			...
			if (Modifier.isStatic(flags)) {
				// MembersOrderPreferenceCache.STATIC_INIT_INDEX
				return getMemberCategory(6);
			} else {
				// MembersOrderPreferenceCache.INIT_INDEX
				return getMemberCategory(4);
			}
		}
		case ASTNode.TYPE_DECLARATION:

 ...
 *
 */
package com.tlcsdm.eclipse.methodsorter.sorter;
...
import com.tlcsdm.eclipse.methodsorter.engine.CompilationUnitSorter;
import com.tlcsdm.eclipse.methodsorter.preferences.IPreferences;
import com.tlcsdm.eclipse.methodsorter.preferences.PreferenceManager;
import com.tlcsdm.eclipse.methodsorter.sorter.callgraph.ASTUtils;
import com.tlcsdm.eclipse.methodsorter.sorter.callgraph.CallGraphExtractor;
import com.tlcsdm.eclipse.methodsorter.sorter.callgraph.CallGraphNode;
import com.tlcsdm.eclipse.methodsorter.sorter.callgraph.CallGraphNode.CallGraphNodeComparator;
import com.tlcsdm.eclipse.methodsorter.sorter.callgraph.SubCallGraphExtractor;
import com.tlcsdm.eclipse.methodsorter.sorter.cluster.ClusterComparator;
import com.tlcsdm.eclipse.methodsorter.sorter.cluster.ClusterGraphExtractor;
import com.tlcsdm.eclipse.methodsorter.sorter.cluster.ClusterNode;
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.BodyDeclarationComparator;
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.ComparatorBuilder;
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.Signature;
import com.tlcsdm.eclipse.methodsorter.sorter.comparator.StackableSignatureComparator;
...
public class CleanCodeMethodSorter implements IMethodSorter {
	...
	/**
	 * @param preferences provides preferences used in this class instance.
	 */
	...
	/**
	 * Instantiation uses the User-configurable preferences stored in the Eclipse
	 ...
	 *
	 * @param subGraph
	 * @param ast      AST which was used when creating this graph or its superGraph
	 */
	...
	/**
	 * Sorts the callGraph (aka a list of {@link CallGraphNode} aka "WorkingList")
	 * following some empirically unverified heuristics. The working list provides
	 * an list of start-points for the invocation order.
	 */
	...
	/**
	 * Sorts the callGraph (aka a list of {@link CallGraphNode} aka "WorkingList")
	 * following the order found in the source, hoping the developer had some clues
	 * about sorting his/her methods already. The working list provides an list of
	 * start-points for the invocation order.
	 */
	...
	/**
	 * @return a signatureComparator representing the sorting information needed to
	 *         sort the methods in the CU *except* clustering of methods.
	 */
	 ...
	 *
	 * @param signatureComparator represents the ordering which is derived from
	 *                            invovation ordering (and reachability). May be
	 *                            wrapped into a {@link ClusterComparator} when
	 *                            clustering is activated in the preferences.
	...
	/**
	 * @param signatureComparator used to sort the subgraphs (clusters) in the
	 *                            callGraph.
	...
	/**
	 * Creates a invocation-subGraph consisting only of nodes and edges between the
	 * given collection of nodes.
	 *
	 * @param nodes
	 * @return a list of {@link CallGraphNode}s connected to invocation graph. The
	 *         nodes are sorted following source position order.
	 *
	 */
